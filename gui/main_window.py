...
class VoiceAssistantGUI(QMainWindow):
    ...
    def start_voice_recording(self):
        if self.assistant:
            try:
                audio_data = self.assistant.audio_handler.record_audio(duration=5, apply_filters=True)
                self.voice_viz.setText('🔄 Processing...')
                result = self.assistant.speech_recognizer.transcribe(audio_data)
                ...
                self.add_message("LYRA", f"Voice recognition error: {str(e)}")
            finally:
                self.stop_listening()
    ...
    def update_assistant_state(self, state):
        ...
        state_messages = {
            "ready": "🎤 Ready - Click mic or type a command",
        ...
        if state in state_messages:
            ...
            if state == "ready":
                ...
                self.statusBar().showMessage("Processing and responding...")

    def set_continuous_mode(self, enabled):
        """Enable or disable continuous listening mode"""
        self.is_continuous_mode = enabled
        if enabled:
            self.statusBar().showMessage("Continuous listening with voice isolation: ACTIVE")
            self.voice_viz.setText("🎤 Continuous listening with voice isolation - Speak naturally!")
        else:
            self.statusBar().showMessage("Continuous listening mode: INACTIVE")
            self.voice_viz.setText("🎤 Click microphone to speak or type a command below")
